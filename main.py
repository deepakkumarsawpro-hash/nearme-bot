...
# Simple memory storage (Production ke liye database use karein)
user_states = {}
...
def webhook():
    ...
    try:
        ...
        if 'interactive' in msg_data:
            selected = msg_data['interactive']['list_reply']['id']
            # State logic yahan aayega (Buyer/Seller -> Location -> Category -> Subcat)
            if selected == "buyer":
                send_interactive_menu(sender_id, "Location", "अपनी लोकेशन चुनें:", 
                                     [{"id": "ranchi", "title": "Ranchi"}, {"id": "bokaro", "title": "Bokaro"}])
            elif selected == "ranchi":
